...
import { TonClient4, Address, WalletContractV4, WalletContractV5R1 } from '@ton/ton';
import { SupportJetton, TrainJetton } from '../build/jetton_train/tact_TrainJetton';
...
export async function run() {
    ...
    const htlcJettonWallet = Address.parse(process.env.htlcJettonWalletAddress!);

    const supportJettonMessage: SupportJetton = {
        $$type: 'SupportJetton',
        jettonMaster: jettonMaster,
        htlcJettonWallet: htlcJettonWallet,
    };

    console.log('Sending SupportJetton message...');
