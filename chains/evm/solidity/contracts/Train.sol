...
contract Train is ReentrancyGuard, EIP712 {
  ...
  function lockBatch(
    ...
    uint48[] calldata rewardTimelocks,
    uint48[] calldata timelocks,
    address payable[] calldata srcReceivers,
    string[] calldata srcAssets,
    string[] calldata dstChains,
    string[] calldata dstAddresses,
    string[] calldata dstAssets,
    ...
    if (
      hashlocks.length != n ||
      rewards_.length != n ||
      rewardTimelocks.length != n ||
      timelocks.length != n ||
      srcReceivers.length != n ||
      srcAssets.length != n ||
      dstChains.length != n ||
      dstAddresses.length != n ||
      dstAssets.length != n ||
      values.length != n
    ) revert();
    ...
    for (uint256 i; i < n; ) {
      ...
      uint256 rew = rewards_[i];
      if (val == 0 || val <= rew) revert FundsNotSent();
      if (block.timestamp + 1800 > timelocks[i]) revert InvalidTimelock();
      if (rewardTimelocks[i] > timelocks[i] || rewardTimelocks[i] <= block.timestamp) revert InvaliRewardTimelock();
      ...
      if (rew != 0) {
        rewards[Id] = Reward(rew, rewardTimelocks[i]);
      }
    ...
    if (total != msg.value) revert FundsNotSent();
    return Ids;
  }
