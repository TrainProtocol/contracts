...
contract Train is ReentrancyGuard, EIP712 {
  ...
  function rollover(
    ...
    uint256 newAmount,
    uint256 newReward,
    uint48 newRewardTimelock,
    uint48 newTimelock,
    address payable newSrcReceiver,
    string calldata newSrcAsset,
    string calldata newDstChain,
    string calldata newDstAddress,
    string calldata newDstAsset
  ) external payable nonReentrant returns (bytes32) {
    if (!hasHTLC(oldId)) revert HTLCNotExists();
    if (hasHTLC(newId)) revert HTLCAlreadyExists();

    HTLC storage oldH = contracts[oldId];
    if (oldH.claimed == 2 || oldH.claimed == 3) revert AlreadyClaimed();
    if (oldH.timelock > block.timestamp) revert NotPassedTimelock();
    if (msg.sender != oldH.sender) revert NoAllowance();

    if (block.timestamp + 1800 > newTimelock) revert InvalidTimelock();
    if (newReward != 0) {
      if (newRewardTimelock > newTimelock || newRewardTimelock <= block.timestamp) revert InvaliRewardTimelock();
    }

    uint256 pool = oldH.amount;
    Reward memory oldRw = rewards[oldId];
    if (oldRw.amount != 0) {
      pool += oldRw.amount;
      delete rewards[oldId];
    }
    pool += msg.value;

    uint256 totalRequired = newAmount + newReward;
    if (pool < totalRequired) revert FundsNotSent();

    oldH.claimed = 2;
    oldH.amount = 0;

    contracts[newId] = HTLC(
      newAmount,
      newHashlock,
      uint256(1),
      payable(msg.sender),
    ...
    if (newReward != 0) {
      rewards[newId] = Reward(newReward, newRewardTimelock);
    }

    uint256 change = pool - totalRequired;
    if (change != 0) {
      (bool ok, ) = payable(msg.sender).call{ value: change, gas: 10000 }('');
      ok;
    }

    emit TokenRefunded(oldId);
    emit TokenLocked(
      newId,
      newHashlock,
      newDstChain,
