...
import {PropertiesAsserts} from "../../invariant/utils/PropertiesAsserts.sol";
import {Train} from "../../../src/tempo/Train.sol";

/// @notice Invariants for the Train HTLC escrow. See test/invariant/PROPERTIES.md for the spec, IDs
///         and guarantees (the properties themselves are chain-agnostic; only the Train import and the
///         dropped `userLockFor` attribution note differ from the shared suite).
...
abstract contract Properties is PropertiesAsserts, Snapshots {
    ...
    ///         scale-safe getters) and ADV-18. Every user lock is owned by exactly one actor (the
    ///         `userLock` caller — Tempo's Train has no `userLockFor` attribution path), so the
    ///         per-owner totals must sum to the full registry count.
