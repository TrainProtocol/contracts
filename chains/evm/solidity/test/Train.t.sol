...
contract TrainTest is Test {
    ...
    function _defaultDestination() internal pure returns (Train.DestinationInfo memory) {
        return
            Train.DestinationInfo({
                dstChain: "ETH",
                dstAddress: "0xDstAddr",
                dstAmount: 1,
                dstToken: "ETH"
            });
    }

    function _defaultUserParams(
        uint256 amount,
        address tokenAddr
    ...
    function test_solverLock_WithReward_StoresLock() public {
        ...
        Train.SolverLockParams memory params = _defaultSolverParams(
            amount,
            NATIVE_ETH,
            reward,
            NATIVE_ETH
    ...
    function test_solverLock_ETH_ETH_BalanceChanges() public {
        ...
        assertEq(address(train).balance, contractETHBefore + amount + reward, "Contract ETH should increase by amount+reward");
    }
    ...
    function test_redeemSolver_ETH_ETH_BeforeRewardTimelock_BalanceChanges() public {
        ...
        assertEq(address(train).balance, contractETHBefore - amount - reward, "Contract ETH should decrease by amount+reward");
    }
    ...
    function test_redeemSolver_ETH_ETH_AfterRewardTimelock_BalanceChanges() public {
        ...
        // Verify balance changes
        assertEq(receiver.balance, receiverBalanceBefore + amount, "Receiver should get amount");
        assertEq(rewardRecipient.balance, rewardRecipientBalanceBefore, "RewardRecipient should get nothing (after rewardTimelock)");
    ...
    function test_redeemSolver_ETH_ERC20_BeforeRewardTimelock_BalanceChanges() public {
        ...
        assertEq(token.balanceOf(rewardRecipient), rewardRecipientTokenBefore + reward, "RewardRecipient should get token reward");
    ...
    function test_redeemSolver_ETH_ERC20_AfterRewardTimelock_BalanceChanges() public {
        ...
        // Verify balance changes
        assertEq(receiver.balance, receiverETHBefore + amount, "Receiver should get ETH amount");
        assertEq(token.balanceOf(rewardRecipient), rewardRecipientTokenBefore, "RewardRecipient should get no tokens (after rewardTimelock)");
        assertEq(token.balanceOf(relayer), relayerTokenBefore + reward, "Relayer should get token reward (after rewardTimelock)");
        assertEq(address(train).balance, contractETHBefore - amount, "Contract ETH should decrease by amount");
        assertEq(token.balanceOf(address(train)), contractTokenBefore - reward, "Contract token should decrease by reward");
    }
    ...
    function test_redeemSolver_ERC20_ETH_AfterRewardTimelock_BalanceChanges() public {
        ...
        // Verify balance changes
        assertEq(token.balanceOf(receiver), receiverTokenBefore + amount, "Receiver should get token amount");
        assertEq(rewardRecipient.balance, rewardRecipientETHBefore, "RewardRecipient should get no ETH (after rewardTimelock)");
    ...
    function test_solverLock_ERC20_ERC20_SameToken_BalanceChanges() public {
        ...
        assertEq(token.balanceOf(solver), solverTokenBefore - amount - reward, "Solver token should decrease by amount+reward");
        assertEq(token.balanceOf(address(train)), contractTokenBefore + amount + reward, "Contract token should increase by amount+reward");
    }
    ...
    function test_redeemSolver_ERC20_ERC20_SameToken_BeforeRewardTimelock_BalanceChanges() public {
        ...
        assertEq(token.balanceOf(rewardRecipient), rewardRecipientTokenBefore + reward, "RewardRecipient should get token reward");
        assertEq(token.balanceOf(relayer), relayerTokenBefore, "Relayer should get no tokens (before rewardTimelock)");
        assertEq(token.balanceOf(address(train)), contractTokenBefore - amount - reward, "Contract token should decrease by amount+reward");
    }
    ...
    function test_refundSolver_ERC20_ERC20_SameToken_BalanceChanges() public {
        ...
        // Verify balance changes
        assertEq(token.balanceOf(solver), solverTokenBefore + amount + reward, "Solver should get token amount+reward back");
        assertEq(token.balanceOf(address(train)), contractTokenBefore - amount - reward, "Contract token should decrease by amount+reward");
    }
    ...
    function test_solverLock_ERC20_ERC20_DifferentTokens_BalanceChanges() public {
        ...
        assertEq(token.balanceOf(address(train)), contractToken1Before + amount, "Contract token1 should increase by amount");
        assertEq(token2.balanceOf(address(train)), contractToken2Before + reward, "Contract token2 should increase by reward");
    }
    ...
    function test_redeemSolver_ERC20_ERC20_DifferentTokens_BeforeRewardTimelock_BalanceChanges() public {
        ...
        assertEq(token2.balanceOf(rewardRecipient), rewardRecipientToken2Before + reward, "RewardRecipient should get token2 reward");
        assertEq(token2.balanceOf(relayer), relayerToken2Before, "Relayer should get no token2 (before rewardTimelock)");
        assertEq(token.balanceOf(address(train)), contractToken1Before - amount, "Contract token1 should decrease by amount");
        assertEq(token2.balanceOf(address(train)), contractToken2Before - reward, "Contract token2 should decrease by reward");
    }
    ...
    function test_redeemSolver_ERC20_ERC20_DifferentTokens_AfterRewardTimelock_BalanceChanges() public {
        ...
        // Verify balance changes
        assertEq(token.balanceOf(receiver), receiverToken1Before + amount, "Receiver should get token1 amount");
        assertEq(token2.balanceOf(rewardRecipient), rewardRecipientToken2Before, "RewardRecipient should get no token2 (after rewardTimelock)");
        assertEq(token2.balanceOf(relayer), relayerToken2Before + reward, "Relayer should get token2 reward (after rewardTimelock)");
        assertEq(token.balanceOf(address(train)), contractToken1Before - amount, "Contract token1 should decrease by amount");
        assertEq(token2.balanceOf(address(train)), contractToken2Before - reward, "Contract token2 should decrease by reward");
    }
    ...
    function test_redeemSolver_ERC20_ERC20_SameToken_RecipientIsRewardRecipient_OptimizedTransfer() public {
        ...
        // Receiver gets both amount and reward (optimized single transfer)
        assertEq(token.balanceOf(receiver), receiverTokenBefore + amount + reward, "Receiver should get token amount+reward combined");
        assertEq(token.balanceOf(address(train)), contractTokenBefore - amount - reward, "Contract token should decrease by amount+reward");
    }
    ...
    function test_solverLock_ERC20_ZeroReward_BalanceChanges() public {
        ...
        assertEq(token.balanceOf(address(train)), contractTokenBefore + amount, "Contract token should increase by amount only");
    }
