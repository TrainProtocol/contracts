...
contract PayoutCurveTest is Test {
  ...
  function setUp() public {
    ...
    vm.deal(initiator, 1_000_000 ether);
    vm.deal(solver, 1_000_000 ether);
  ...
  function test_userLock_constantCurve_stores() public {
    vm.prank(initiator);
    train.userLock{ value: 1 ether }(_user(1 ether, NATIVE_ETH, address(constCurve)), _dst(), '', '');
  ...
  function test_redeemUser_ETH_constantCurve_fullPayout() public {
    vm.prank(initiator);
    train.userLock{ value: 1 ether }(_user(1 ether, NATIVE_ETH, address(constCurve)), _dst(), '', '');
    vm.warp(block.timestamp + 100_000);
    uint256 r = recipient.balance;
    uint256 f = refundTo.balance;
    vm.prank(relayer);
    train.redeemUser(hashlock, SECRET);
    assertEq(recipient.balance - r, 1 ether);
    assertEq(refundTo.balance - f, 0);
  }
  ...
  function test_redeemSolver_constantCurve_fullPayout_rewardIntact() public {
    vm.prank(solver);
    uint256 idx = train.solverLock{ value: 1.1 ether }(
      _solver(1 ether, NATIVE_ETH, 0.1 ether, NATIVE_ETH, address(constCurve)), _dst(), ''
    );
    vm.warp(block.timestamp + 600);
    uint256 r = recipient.balance;
    uint256 f = refundTo.balance;
    uint256 rr = rewardRecipient.balance;
    vm.prank(relayer);
    train.redeemSolver(hashlock, idx, SECRET);
    assertEq(recipient.balance - r, 1 ether);
    assertEq(refundTo.balance - f, 0);
    assertEq(rewardRecipient.balance - rr, 0.1 ether);
  }
  ...
  function test_userLock_revertsOnInvalidPayoutCurve() public {
    ...
    train.userLock{ value: 1 ether }(_user(1 ether, NATIVE_ETH, address(0xdead)), _dst(), '', '');
  }
  ...
  function test_userLock_revertsWhenCurveReturnsFalseInterface() public {
    ...
    vm.prank(initiator);
    vm.expectRevert(Train.InvalidPayoutCurve.selector);
    train.userLock{ value: 1 ether }(_user(1 ether, NATIVE_ETH, bad), _dst(), '', '');
  }
  ...
  function test_redeemUser_ETH_curveExcess_splitsAndEmits() public {
    vm.prank(initiator);
    train.userLock{ value: 100 ether }(_user(100 ether, NATIVE_ETH, address(half)), _dst(), '', '');
    uint256 r = recipient.balance;
    uint256 f = refundTo.balance;
    vm.expectEmit(true, false, false, true);
    emit Train.UserRedeemed(hashlock, relayer, SECRET, 50 ether, 50 ether);
    vm.prank(relayer);
    train.redeemUser(hashlock, SECRET);
    assertEq(recipient.balance - r, 50 ether);
    assertEq(refundTo.balance - f, 50 ether);
  }
  ...
  function testFuzz_redeemUser_curveExcess_conservation(uint256 amount, uint256 numr, uint256 denr) public {
    ...
    vm.deal(initiator, amount);
    vm.prank(initiator);
    train.userLock{ value: amount }(_user(amount, NATIVE_ETH, address(c)), _dst(), '', '');
    uint256 r = recipient.balance;
    uint256 f = refundTo.balance;
    vm.prank(relayer);
    train.redeemUser(hashlock, SECRET);
    uint256 rd = recipient.balance - r;
    uint256 fd = refundTo.balance - f;
