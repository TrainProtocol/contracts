...
/// @dev Create handlers mint a fresh harness-owned secret and record the lock so redeem/refund
...
abstract contract TrainHandler is Properties {
    ...
    function handler_userLockFor(uint256 callerSeed, uint256 userSeed, uint256 amountSeed, uint256 tldSeed) public {
        ...
        (uint256 s, bytes32 hl) = _freshLock();
        vm.prank(caller);
        try train.userLockFor(user, _userParams(hl, amount, tld), _dst(), "", "") {
            userHashlocks.push(hl);
            userSecretOf[hl] = s;
            _propUserLockCreated(hl, amount);
        } catch {}
