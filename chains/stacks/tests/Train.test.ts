...
describe("Train Contract Test", () => {
  it("ensures commit function works correctly and  fails with zero value, past timelock", () => {
    ...
    const contractNonce = cvToValue(simnet.getDataVar("Train", "contract-nonce"));
    const seed = cvToValue(simnet.getDataVar("Train", "seed"));
    ...
    const contractPrincipal = 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.Train';
...
it("ensures addLock function works correctly", () => {
  ...
  const addLock = simnet.mineBlock([tx.callPublicFn("Train", "add-lock", [
      Cl.uint(expectedContractId),
      Cl.buffer(randomHashlock),
      Cl.uint(futureTimelock + BigInt(3600))
    ], address1)]);
...
it("fails add-lock if the HTLC contract does not exist", () => {
  ...
  const block = simnet.mineBlock([
    tx.callPublicFn("Train", "add-lock", [
      Cl.uint(fakeId), 
      Cl.buffer(randomHashlock),
      Cl.uint(BigInt(3600))
    ], address1)
...
it("fails add-lock if the timelock has already passed", () => {
 ...
 it("ensures add-lock-sig function works correctly", () => {
    ...
    const addLockSig = simnet.mineBlock([
      tx.callPublicFn("Train", "add-lock-sig", [
        Cl.uint(contractId),
        Cl.buffer(randomHashlock),
        Cl.uint(futureTimelock + BigInt(3600)),
