...
// Reentrancy guard for use in Sway contracts.
//
// Vendored verbatim from `sway_libs::reentrancy` (sway-libs v0.25.2,
// commit f43d8b4aacf60867989408991414958f8eab28c0). The `sway_libs`
// dependency was dropped because no released `sway_libs` version is
// compatible with forc/std > 0.67 (its transitive `sway-standards` dep
// fails to compile), and `reentrancy_guard` was the only symbol this
// contract used from it. Behaviour is byte-for-byte identical to the
// upstream implementation; it relies solely on `std` call-frame APIs.
//
